...
class FactExtractor:
    ...
    def extract(
        ...
        *,
        ...
        strict_required_canonicals: bool = True,
    ) -> ExtractionResult:
        ...
        if coverage_pct is not None and coverage_pct < min_coverage_pct:
            result.quality["gate"] = "review"
            result.quality["issues"].append("low_coverage")

        paragraph_scores = [
            float(m["score"])
            for m in (matched_paragraphs or [])
            if isinstance(m.get("score"), (int, float))
        ]
        if paragraph_scores:
            result.quality["matched_paragraph_score_max"] = max(paragraph_scores)
            result.quality["matched_paragraph_score_avg"] = round(
                sum(paragraph_scores) / len(paragraph_scores), 3
            )
        ...
        for i, raw_fact in enumerate(raw_facts):
            ...
            passes_required, unresolved = self.resolver.validate_fact(merged)
            if strict_required_canonicals and not passes_required:
                result.rejected_facts.append({
                    "fact_index": i,
                    "reason": "unresolved_required_canonicals",
                    "unresolved_fields": unresolved,
                    "fact_id": merged.get("fact_id"),
        ...
        result.quality["accepted_facts"] = len(result.facts)
        result.quality["rejected_facts"] = len(result.rejected_facts)
        if result.rejected_facts:
            result.quality["issues"].append("rejected_facts")
            if result.quality["gate"] == "pass":
                result.quality["gate"] = "review"
        if extracted > 0 and len(result.facts) == 0:
            result.quality["gate"] = "fail"
            result.quality["issues"].append("no_accepted_facts")
