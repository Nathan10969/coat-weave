...
环境变量在 ``qwen-vl-plus`` / ``qwen-vl-max`` / 未来 ``qwen3-vl-*`` 间切换。
...
class QwenVLClient:
    ...
    ``model`` 是字符串，调用方可传任一个：
        - ``qwen-vl-plus`` (figure 默认)
        - ``qwen-vl-max``  (质量更高，更贵)
        - ``qwen-plus``    (table 走的纯文本路径)
        - 未来同 endpoint 的 ``qwen3-vl-*``
    """
    ...
    def _call_vlm(self, prompt: str, image_path: Path | None) -> dict[str, Any]:
        ...
        if image_path is not None:
            ...
            # 纯文本路径默认 qwen-plus — VLM SKU 也可以但便宜的 text 模型够推 HTML 表
