...
class CanonicalResolver:
    ...
    def resolve_optional_slots(
        ...
        # process list of dicts，new writer uses "canonical_id"; legacy facts use "step".
        steps = fact.get("process") or []
        process_resolutions = [
            self.resolve(s.get("canonical_id") or s.get("step"), kind="PROC")
            for s in steps
            if isinstance(s, dict)
        ]
        out["process"] = process_resolutions
        out["process.step"] = process_resolutions  # legacy report key
        return out
