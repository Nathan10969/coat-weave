...
class ModelConfig:
    ...
    - ``qwen_text``: 纯文本模型 (默认 ``qwen3.6-flash``)，用于 fact 抽取和
      :mod:`vlm_describe` 的纯文本路径 (table HTML)。
    - ``qwen_vl``: 视觉语言模型 (默认 ``qwen3.6-flash``)，用于 figure 描述。
    - ``qwen_table_ocr``: 表格图片 OCR / structure recovery 模型
      (默认 ``qwen3.6-flash``)，用于 table image structure recovery。
...
class Settings:
    ...
    def load(cls) -> "Settings":
        ...
        return cls(
            ...
            models=ModelConfig(
                qwen_vl=_env("QWEN_VL_MODEL", "qwen3.6-flash"),
                qwen_text=_env("QWEN_TEXT_MODEL", "qwen3.6-flash"),
                qwen_table_ocr=_env("QWEN_TABLE_OCR_MODEL", "qwen3.6-flash"),
