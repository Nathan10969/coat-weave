...
def is_coating_patent(meta: dict | None) -> bool:
    ...
    if meta is None:
        return True
    gate_status = meta.get("coating_gate_status") or meta.get("coating_gate")
    if gate_status:
        return gate_status != "non_coating"
    if "is_coating_patent" in meta:
        return bool(meta.get("is_coating_patent"))
...
def example_id_for_fact(
    ...
    if isinstance(llm_ex, str) and llm_ex.strip():
        ...
        # 和 BASF "C1" / "I1" 风格)。
        if _PREFIXED_EXAMPLE_ID_RE.match(raw):
            return raw
...
def app_category(canonical_id: str) -> str:
    ...
    if sub in APP_CATEGORIES:
        return sub
    low = value.casefold()
    if "automotive" in low or "clearcoat" in low or "basecoat" in low:
        return "Automotive"
    if "marine" in low or "fouling" in low or "ship" in low:
        return "Marine / fouling control"
    if "powder" in low:
        return "Powder coating"
    if "wind" in low or "blade" in low:
        return "Wind energy"
    if "architectural" in low:
        return "Architectural"
    if "industrial" in low or "protective" in low or "anti_corrosion" in low or "corrosion" in low:
        return "Industrial protective"
    if "packaging" in low or "electrocoat" in low or "aircraft" in low or "wood" in low or "textile" in low:
...
def sub_category(canonical_id: str) -> str:
    ...
    if sub in SUB_CATEGORIES:
        return sub
    low = value.casefold()
    if any(token in low for token in ("metal", "steel", "aluminum", "aluminium", "tinplate", "crs", "conductive")):
        return "Metal"
    if any(token in low for token in ("plastic", "polycarbonate", "perspex", "polypropylene", "pp", "pet")):
        return "Plastic"
    if any(token in low for token in ("mineral", "concrete", "cement", "gypsum", "mortar")):
        return "Mineral"
    if "wood" in low or "plywood" in low:
        return "Wood"
    if "foam" in low:
        return "Foam / bulk article"
    if "wind turbine" in low or "blade" in low:
        return "Composite / wind blade"
    return _human_label(value)


def _human_label(value: str) -> str:
    """Preserve explicit LLM/doc-profile labels instead of hiding them as unknown."""
    value = value.strip()
    if value.startswith(("APP_", "SUB_")):
        value = value[4:]
...
def build_row(
    ...
    for f in facts:
        ...
        raw_substrate = f.get("substrate")
        s = raw_substrate.get("tested") if isinstance(raw_substrate, dict) else raw_substrate
    ...
    app_votes = [v for v in app_votes if v and v != "unknown"]
    sub_votes = [v for v in sub_votes if v and v != "unknown"]
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Build wide coatings CSV from data/units facts.")
    ap.add_argument(
        "--output",
        type=Path,
