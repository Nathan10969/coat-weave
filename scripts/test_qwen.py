...
def main() -> None:
    ...
    try:
        ...
        if reply and "pong" in reply.lower():
            print(f"      ✅ Text endpoint OK")
        else:
            print(f"      ⚠ Unexpected reply (but API works)")
    except Exception as e:
        print(f"      ❌ Text endpoint FAILED: {e}")
        sys.exit(1)

    # === Test 2: VLM model (qwen-vl-plus) ===
    ...
    try:
        ...
        reply = r.choices[0].message.content
        print(f"      ✓ VLM reply: {reply}")
        print(f"      ✅ VLM endpoint OK")
    except Exception as e:
        print(f"      ❌ VLM endpoint FAILED: {e}")
        sys.exit(1)

    print(f"\n✅ All Qwen API smoke tests PASSED")
