...
用法 (Windows PowerShell):
    ...
    python scripts\\run_pipeline.py `
      C:\\path\\to\\patents\\example.pdf

    # 多个特定 PDF
    python scripts\\run_pipeline.py `
      C:\\path\\to\\patents\\example-a.pdf `
      C:\\path\\to\\patents\\example-b.pdf

    # 目录里前 5 篇 (PowerShell 展开)
    $pdfs = (Get-ChildItem C:\\path\\to\\patents\\*.pdf | Select-Object -First 5).FullName
    python scripts\\run_pipeline.py @pdfs

    # 348 全跑 (耐心等 — ~5h, ~¥150)
    $pdfs = (Get-ChildItem C:\\path\\to\\patents\\*.pdf).FullName
    python scripts\\run_pipeline.py @pdfs
...
def run_csv(csv_output: Path | None = None) -> bool:
    ...
    try:
        result = subprocess.run(
            [
                ...
                *(["--output", str(csv_output)] if csv_output else []),
            ],
            cwd=REPO,
