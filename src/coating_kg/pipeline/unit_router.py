...
def classify_unit_route(
    ...
    if not vlm_desc:
        return RouteDecision("extract_facts", "vlm_failed_fallback_to_extract")
    ...
    if not entities and len(description) < SKIP_DESC_LEN_THRESHOLD:
        return RouteDecision("delete", "low_info_decoration")
    ...
    if unit_type == "figure":
        ...
        if subtype in FIGURE_REGISTER_LAYER1:
            return RouteDecision("register_layer1", f"figure_{subtype}")
        # Unknown subtype — defensive: don't lose the figure, send to layer 1
        return RouteDecision("register_layer1", f"figure_unknown_subtype:{subtype}")

    # Rule 6 & 7: table routing
    table_subject = (vlm_desc.get("table_subject") or "其它").strip()
    if table_subject in TABLE_EXTRACT_FACTS:
        return RouteDecision("extract_facts", f"table_{table_subject}")
    if table_subject in TABLE_REGISTER_LAYER1:
        return RouteDecision("register_layer1", f"table_{table_subject}")
    # Unknown table_subject — defensive: keep as visual reference
    return RouteDecision("register_layer1", f"table_unknown_subject:{table_subject}")
...
class RouteAuditLog:
    ...
    HEADER = ["unit_id", "doc_id", "route", "reason", "description_excerpt"]
    ...
    def write(
        ...
        with _file_lock(self.lock_path):
            ...
            with self.path.open("a", encoding="utf-8", newline="") as f:
                ...
                writer.writerow([unit_id, doc_id, decision.route, decision.reason, excerpt])
...
def write_pending_figure(
    ...
    figure_record = {
        ...
        "confidence": 0.9,  # default; tightened in V2.5 with rerank score
    }
